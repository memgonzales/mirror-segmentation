...
# Xception + Proposed Boundary Extraction Module
# ===================================================
...
class PMDLite(nn.Module):
    def __init__(self, training=False):
        ...
        self.m = timm.create_model('gluon_xception65', features_only=True, pretrained=True)
    
        
        self.edge_extract = BFE_Module(2048)
        
        self.edge_predict = nn.Sequential(nn.Conv2d(768, 384, 1, 1, 1), nn.BatchNorm2d(384),
                                nn.ReLU(), nn.Conv2d(384, 1, 3, 1, 1))

        self.contrast_4 = Contrast_Module_Deep(2048,d1=2, d2=4)
        self.contrast_3 = Contrast_Module_Deep(728,d1=4, d2=8) 
        self.contrast_2 = Contrast_Module_Deep(256, d1=4, d2=8) 
        self.contrast_1 = Contrast_Module_Deep(128, d1=4, d2=8)

        self.ra_4 = Relation_Attention(2048, 2048)
        self.ra_3 = Relation_Attention(728, 728)
        self.ra_2 = Relation_Attention(256, 256)
        self.ra_1 = Relation_Attention(128, 128)

        self.up_4 = nn.Sequential(nn.ConvTranspose2d(2048, 512, 4, 2, 1), nn.BatchNorm2d(512), nn.ReLU())
        self.up_3 = nn.Sequential(nn.ConvTranspose2d(728, 182, 4, 2, 1), nn.BatchNorm2d(182), nn.ReLU())
        self.up_2 = nn.Sequential(nn.ConvTranspose2d(256, 64, 4, 2, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.up_1 = nn.Sequential(nn.ConvTranspose2d(128, 32, 4, 2, 1), nn.BatchNorm2d(32), nn.ReLU())

        self.cbam_4 = CBAM(512)
        self.cbam_3 = CBAM(182)
        self.cbam_2 = CBAM(64)
        self.cbam_1 = CBAM(32)

        self.layer4_predict = nn.Conv2d(512, 1, 3, 1, 1)
        self.layer3_predict = nn.Conv2d(182, 1, 3, 1, 1)
