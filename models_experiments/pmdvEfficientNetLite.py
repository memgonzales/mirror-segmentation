...
# EfficientNetLite + Proposed Boundary Extraction Module
# ===================================================
...
class PMDLite(nn.Module):
    def __init__(self, training=False):
        ...
        self.m = timm.create_model('tf_efficientnet_lite4', features_only=True, pretrained=True)
        
        self.edge_extract = BFE_Module(2048)
        
        self.edge_predict = nn.Sequential(nn.Conv2d(176, 88, 1, 1, 1), nn.BatchNorm2d(88),
                                nn.ReLU(), nn.Conv2d(88, 1, 3, 1, 1))

        self.contrast_4 = Contrast_Module_Deep(448,d1=2, d2=4)
        self.contrast_3 = Contrast_Module_Deep(160,d1=4, d2=8)
        self.contrast_2 = Contrast_Module_Deep(56, d1=4, d2=8)
        self.contrast_1 = Contrast_Module_Deep(32, d1=4, d2=8)

        self.ra_4 = Relation_Attention(448, 448)
        self.ra_3 = Relation_Attention(160, 160)
        self.ra_2 = Relation_Attention(56, 56)
        self.ra_1 = Relation_Attention(32, 32)

        self.up_4 = nn.Sequential(nn.ConvTranspose2d(448, 112, 4, 2, 1), nn.BatchNorm2d(112), nn.ReLU())
        self.up_3 = nn.Sequential(nn.ConvTranspose2d(160, 40, 4, 2, 1), nn.BatchNorm2d(40), nn.ReLU())
        self.up_2 = nn.Sequential(nn.ConvTranspose2d(56, 14, 4, 2, 1), nn.BatchNorm2d(14), nn.ReLU())
        self.up_1 = nn.Sequential(nn.ConvTranspose2d(32, 8, 4, 2, 1), nn.BatchNorm2d(8), nn.ReLU())

        self.cbam_4 = CBAM(112)
        self.cbam_3 = CBAM(40)
        self.cbam_2 = CBAM(14)
        self.cbam_1 = CBAM(8)

        self.layer4_predict = nn.Conv2d(112, 1, 3, 1, 1)
        self.layer3_predict = nn.Conv2d(40, 1, 3, 1, 1)
        self.layer2_predict = nn.Conv2d(14, 1, 3, 1, 1)
        self.layer1_predict = nn.Conv2d(8, 1, 3, 1, 1)
